...
from scheduling.services import ensure_series_occurrences
...
class TherapistTimeOffViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if therapist is not None:
            ensure_series_occurrences(therapist, range_start=start_raw, range_end=end_raw)
        return super().list(request, *args, **kwargs)

    def get_queryset(self):
        queryset = (
            TherapistTimeOff.objects.select_related("therapist", "therapist__user", "series")
    ...
    def perform_destroy(self, instance):
        ...
        if scope == "series":
            if not instance.series_id:
                raise ValidationError({"scope": "This time off does not belong to a recurring series."})
